...
import { UserBrand, UserRoleId } from "@/types/user.types";
import { v4 } from "uuid";
...
export function MediaBulkActions({
  ...
  const handleProductExtraction = async () => {
    ...
    try {
      ...
      console.log("✅ Product extraction started:", {
...
const getStatusCountMessage = () => {
  ...
  selectedItems.forEach((item: GalleryItemResponse) => {
        ...
        (moveAction === "source" && targetSource)) && (
          <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
            ...
            <p className="text-sm text-blue-700">
              ...
              {moveAction === "brand" && (
                <>
                  {brands.find((b: UserBrand) => b.id === targetBrandId)?.name}
              ...
              {moveAction === "campaign" && targetCampaignId !== "none" && (
                <>
                  {
                    availableCampaigns.find((c: any) => c.id === targetCampaignId)
                      ?.title
                  }
                  {!fromSameBrand && (
                    <span className="block text-xs mt-1 text-blue-600">
...
return (
  <TooltipProvider>
    ...
    <ReusableAlertDialog
      ...
      title={`Move Assets - ${moveAction === "brand"
        ? "Brand"
        : moveAction === "campaign"
          ? "Campaign"
          : "Source"
